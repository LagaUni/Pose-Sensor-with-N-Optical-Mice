...
void loop() {
  // Espera hasta que ambos sensores estén listos
  if (!sensor1Ready || !sensor2Ready) {
    return; // Espera hasta que ambos sensores estén listos
  }

  // Procesa y muestra datos para ambos sensores una vez que ambos estén listos
...
void processSensorData(int sensorNumber, int SCLK, int SDIO, long &x) {
  ...
  if (motion == 0x81) {
    ...
    if (sensorNumber == 1) {
      ...
      y2 += delta_y * 3; // Actualiza la posición acumulativa global para el sensor 2
      cx = (-0.0254 * x2);
      cy = (-0.0254 * y2) ;
